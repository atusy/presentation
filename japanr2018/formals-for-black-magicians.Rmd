...
`AssignInNamespace("b", 2, getNamespace("japanr2018"))` 
...
# 応用1 <br> ggplot2で密度分布の信頼区間を描写する {#stat-ci}

## `ggproto` で `StatCI` を作る {#ggproto}
...
## `ggplot2::stat_density` を <br> `stat_ci` に改造する {#stat-ci-goal}
...
`ggplot2::StatDensity` から、 \
引数の `StatDensity = StatCI` に摩り替える。



```{r stat-ci, echo = FALSE, fig.height = 2}
library(ggplot2)
...
[実際には `StatDensity` 引数を弄れないよう、もう一工夫した。](https://github.com/atusy/ggAtusy/blob/master/R/stat_ci.R)
...
# 応用2 <br> `skimr::inline_hist()` の <br /> ビン数を変更 {#modify-inline-hist}

## 内部変数 `optioins` を弄る {#modify-options-in-skimr}
...
## 動的に `optioins` を弄る {#dynamic-options}
...
formals(inline_hist)[c("n", "options")] = alist(n = 10, options = .options(n))
inline_hist(iris$Petal.Length)
inline_hist(iris$Petal.Length, 20)
inline_hist(iris$Petal.Length, 30)
```
