...
library(patchwork)
library(knitr)
opts_chunk$set(dev = 'CairoSVG', fig.show = 'hold', cache = TRUE, message = FALSE)
formals(stat_bin)$bins <- 30
assign
```
...
```{r}
library(tidyr)
longcars <- gather(minicars, key = var, value = val, -am, -vs, -mpg)
```
...
# `facet_grid` {#facet-grid-intro}

## 注目する変数でグラフを表形式に分割

```{r, fig.height = 3}
ggplot(minicars, aes(x = wt, y = mpg)) + geom_point() +
  facet_grid(vs ~ am) # or facet_grid(vars(vs), vars(am))
```
